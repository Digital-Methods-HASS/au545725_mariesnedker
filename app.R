    ...
    titlePanel("App: Multiple word search in pdf's"),
        ...
        mainPanel(
          ...
          "Documents in folder:",
            ...
            sidebarPanel(textInput("input1", label="Search word 1", placeholder="Please enter search word 1 or space"), tableOutput("output1"), width=10),
            sidebarPanel(textInput("input2", label="Search word 2", placeholder="Please enter search word 1 or space"), tableOutput("output2"), width=10),
            sidebarPanel(textInput("input3", label="Search word 3", placeholder="Please enter search word 1 or space"), tableOutput("output3"), width=10), 
            sidebarPanel(textInput("input4", label="Search word 4", placeholder="Please enter search word 1 or space"), tableOutput("output4"), width=10), 
            sidebarPanel(textInput("input5", label="Search word 5", placeholder="Please enter search word 1 or space"), tableOutput("output5"), width=10), 
            
            sidebarPanel(("Documents where all search words appear"), tableOutput("output6"), width = 10)
        )
)
