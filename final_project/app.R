...
ui <- fluidPage(
    ...
    titlePanel("App: Multiple word search in PDF's"),
        ...
        mainPanel(
          ...
          h4("In this app you can search for multiple words in pdf documents. All your pdf documents need to be in the same folder as the app. 
             Please insert your search word or space if you don't need all search options. For each search word you will see in which documents it appears. 
             At the bottom of the page you will see in which documents all your search words appear at the same time. Have fun."),
          tabsetPanel(          
...
server <- function(input, output) {
    ...
    # Filters out the rows where the particular word doesn't appear. 
    ...
    # Prints the document(s) in which all search words appear at the same time.
